...
parser = argparse.ArgumentParser(description='Do stuff with files.', prog='pyscan.py',
...
parser.add_argument('--ignore_dirs', nargs="?", default=None, help="Ignore certain directories")
...
def main():
    global outfile,infile,tosearch,targettype,searchrules,extfilter,verbose,recursive,errorhandling,recursive,resultdict,numthreads,threads,blacklist,bad_dirs
    ...
    try:
        ...
        blacklist = []
    try:
        bad_dirs = args.ignore_dirs.split(",")
    except:
        bad_dirs = []
        ...
        for filename in filenames:
            ...
            for bdir in bad_dirs:
                if bdir in dirpath:
                    ig_dir = True
            if not ig_dir:
                flist.add('%s/%s'%(dirpath,filename))
        if not recursive:
            break
...
def dumpresults():
    ...
    with open(outfile,'w') as f:
        for key,values in resultdict.iteritems():
            if len(values) > 0:
                print("Found %s instances of %s" % (len(values), key))
                f.write('%s\n'%key)
                for value in values:
                    f.write('%s\n'%value)
